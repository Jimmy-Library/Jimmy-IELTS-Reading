(function (global) {
    ...
    LegacyExamListView.prototype._createExamElement = function _createExamElement(exam, index, options) {
        ...
        var titleText = document.createTextNode(exam.title || '');
        title.appendChild(titleText);

        // 查看做题记录按钮
        var recordBtn = this._createElement('button', {
            className: 'exam-record-btn',
            type: 'button',
            title: '查看做题记录',
            dataset: { action: 'view-records', examId: exam.id },
            style: 'background:none;border:none;cursor:pointer;font-size:0.9rem;padding:0 4px;opacity:0.6;line-height:1;'
        }, '\uD83D\uDCCB');
