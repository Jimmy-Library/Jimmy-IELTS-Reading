class PracticeRecordModal {
    ...
    teardownEventListeners() {
        ...
        if (this.modalElement && this.boundExportClickHandler) {
            this.modalElement.removeEventListener('click', this.boundExportClickHandler);
        }
        ...
        this.boundReplayClickHandler = null;
        this.boundReplayKeyHandler = null;
        this.boundExportClickHandler = null;
    }
    ...
    setupEventListeners(modal) {
        ...
        this.boundExportClickHandler = (event) => {
            ...
            if (action !== 'export-record-pdf' && action !== 'export-record-image') return;
            event.preventDefault();
            this.handleExportAction(action);
        };
        modal.addEventListener('click', this.boundExportClickHandler);
    }

    /**
     * 处理记录详情底部"导出 PDF / 图片"按钮点击
     */
    async handleExportAction(action) {
        const record = this.currentRecord;
        if (!record) {
            if (typeof window.showMessage === 'function') {
                window.showMessage('未找到记录数据', 'error');
            }
            return;
        }

        // 确保 MarkdownExporter / PdfExporter 可用
        if (!window.markdownExporter && typeof window.MarkdownExporter === 'function') {
            try { window.markdownExporter = new window.MarkdownExporter(); } catch (_) { /* ignore */ }
        }
        if (typeof window.PdfExporter !== 'function') {
            if (typeof window.showMessage === 'function') {
                window.showMessage('导出模块未就绪,请稍后重试', 'warning');
            }
            return;
        }
        ...
        try {
            if (action === 'export-record-pdf') {
                await window.pdfExporter.exportSingleRecordAsPdf(record);
            } else if (action === 'export-record-image') {
                await window.pdfExporter.exportRecordAsImage(record);
            }
        } catch (error) {
            console.error('[PracticeRecordModal] 导出失败:', error);
            if (typeof window.showMessage === 'function') {
                window.showMessage('导出失败:' + (error.message || '未知错误'), 'error');
            }
        }
    }
    ...
    async handleReviewAction() {
        ...
        try {
            if (window.app && typeof window.app.openPracticeRecordReplay === 'function') {
                ...
                // 降级：尝试直接打开回顾
                if (typeof window.showMessage === 'function') {
                    window.showMessage('回顾功能模块未加载，请刷新页面后重试', 'warning');
                }
            }
        } catch (error) {
            console.error('[PracticeRecordModal] 回顾记录失败:', error);
            if (typeof window.showMessage === 'function') {
                window.showMessage('无法打开回顾: ' + (error.message || '未知错误'), 'error');
            }
        }
    }
        ...
        return `
            <div id="${this.modalId}" class="modal-overlay">
                <div class="modal-container">
                    ...
                    <div class="modal-footer record-export-bar">
                        <button type="button" class="btn record-review-btn" data-action="review-record">
                            🔍 回顾记录
                        </button>
                        <span class="record-export-hint">导出当前这条记录:</span>
                        <button type="button" class="btn record-export-btn" data-action="export-record-pdf">
                            📄 导出本条 PDF
                        </button>
                        <button type="button" class="btn record-export-btn record-export-btn--image" data-action="export-record-image">
                            🖼️ 导出本条图片
                        </button>
                    </div>
                </div>
