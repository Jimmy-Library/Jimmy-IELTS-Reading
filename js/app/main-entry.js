(function bootstrapApp(global) {
    ...
    ensureGlobalFunctionAfterGroup('filterByNewSeptember2026', BROWSE_GROUP, function () {
        if (typeof global.showMessage === 'function') {
            global.showMessage('题库筛选模块未就绪', 'warning');
        }
    });
    ...
    ensureGlobalFunctionAfterGroup('filterRecordsByStatus', BROWSE_GROUP, function () {
        if (typeof global.showMessage === 'function') {
            global.showMessage('练习筛选模块未就绪', 'warning');
        }
    });

    ensureGlobalFunctionAfterGroup('filterRecordsByTime', BROWSE_GROUP, function () {
        if (typeof global.showMessage === 'function') {
            global.showMessage('练习筛选模块未就绪', 'warning');
        }
    });
