(function (global) {
   ...
   // 配置与常量
    ...
    const AUGUST_2026_NEW_IDS = [
        'p3-high-161', 'p2-high-09', 'p1-medium-1255',
        'p1-high-1783', 'p3-high-1051', 'p2-low-1823',
    ...
    function applyBrowsePostFilters(exams) {
        ...
        result = applyExamSort(result);
        if (global.__browseNewOnly === 'august-2026' || global.__browseNewOnly === true) {
            const featuredOrder = new Map(AUGUST_2026_FEATURED_IDS.map((id, index) => [id, index]));
            result.sort((a, b) => {
                const aRank = featuredOrder.has(a && a.id) ? featuredOrder.get(a.id) : Number.MAX_SAFE_INTEGER;
                const bRank = featuredOrder.has(b && b.id) ? featuredOrder.get(b.id) : Number.MAX_SAFE_INTEGER;
                return aRank - bRank;
            });
        }
        return result;
   ...
   // 核心功能：加载与渲染
    ...
    function createExamCard(exam, options = {}) {
        ...
        titleText.textContent = exam.title || '';
        title.appendChild(titleText);

        // 查看做题记录按钮
        var recordBtn = document.createElement('button');
        recordBtn.className = 'exam-record-btn';
        recordBtn.type = 'button';
        recordBtn.dataset.action = 'view-records';
        recordBtn.dataset.examId = exam.id;
        recordBtn.title = '查看做题记录';
        recordBtn.setAttribute('aria-label', '查看做题记录');
        recordBtn.textContent = '📋';
        recordBtn.style.cssText = 'background:none;border:none;cursor:pointer;font-size:0.9rem;padding:0 4px;opacity:0.6;line-height:1;';
        title.appendChild(recordBtn);
    ...
    function setupExamActionHandlers() {
        ...
        if (hasDomDelegate) {
            ...
            global.DOM.delegate('click', '[data-action="view-records"]', function (event) {
                invoke(this, event);
            });
    ...
    async function handleViewRecords(examId) {
        if (!examId) return;
        ...
        try {
            ...
            } else if (window.getPracticeRecordsState && typeof window.getPracticeRecordsState === 'function') {
                records = window.getPracticeRecordsState() || [];
            } else {
                try {
            ...
            if (!Array.isArray(records) || records.length === 0) {
                if (typeof global.showMessage === 'function') {
                    global.showMessage('暂无该题目的练习记录', 'info');
                }
                return;
            }
            ...
            matched.sort(function (a, b) {
                return new Date(b.date || b.endTime || 0) - new Date(a.date || a.endTime || 0);
            });
            var latest = matched[0];

            if (window.practiceRecordModal) {
                window.practiceRecordModal.show(latest);
            } else if (typeof global.showMessage === 'function') {
                global.showMessage('记录详情组件未加载，请刷新页面后重试', 'warning');
            }
        } catch (error) {
            console.error('[ExamActions] 查看练习记录失败:', error);
            if (typeof global.showMessage === 'function') {
                global.showMessage('无法加载练习记录: ' + (error && error.message || error), 'error');
            }
        }
    }
