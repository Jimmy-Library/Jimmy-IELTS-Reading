        ...
        _sendSimulationContext(session, examId, targetWindow) {
            ...
            const payload = {
                ...
                data: {
                    ...
                    // 套题全部小节的 examId（按顺序），供题目导航渲染全部 40 题并支持跨篇跳转
                    suiteSequenceExamIds: session.sequence
                        .map(e => (e && e.examId != null) ? String(e.examId) : null)
                        .filter(Boolean),
                    draft,
                    elapsed,
                    globalTimerAnchorMs: session.globalTimerAnchorMs
        ...
        async _handleSimulationNavigate(examId, data, sourceWindow) {
            ...
            try {
                ...
                if (direction === 'next') {
                    ...
                    targetIdx = currentIdx - 1;
                } else if (Number.isInteger(data && data.targetIndex)) {
                    // 题目导航跨篇跳转：直接定位到指定小节
                    targetIdx = data.targetIndex;
                } else {
                    return false;
                }
