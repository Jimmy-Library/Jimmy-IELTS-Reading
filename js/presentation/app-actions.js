(function initAppActions(global) {
    ...
    function exportPracticePdf() {
        ensurePracticeSuite().then(function handleExportReady() {
            // 确保 MarkdownExporter 已初始化(PdfExporter 复用其数据加载与答题表逻辑)
            if (!global.markdownExporter && typeof global.MarkdownExporter === 'function') {
                try {
                    global.markdownExporter = new global.MarkdownExporter();
                } catch (error) {
                    console.error('[AppActions] 初始化 MarkdownExporter 失败:', error);
            ...
            if (typeof global.PdfExporter !== 'function') {
                if (typeof global.showMessage === 'function') {
                    global.showMessage('PDF 导出模块未就绪,请稍后重试', 'warning');
                }
                return;
            }

            // 优先打开选择弹窗,让用户挑选要导出的记录
            if (typeof global.PracticeExportSelector === 'function') {
                try {
                    const selector = new global.PracticeExportSelector();
                    selector.open();
                    return;
                } catch (error) {
                    console.warn('[AppActions] 打开选择弹窗失败,回退到全量导出:', error);
                }
            }

            // 选择器不可用时,回退到全量导出
            try {
                if (!global.pdfExporter || !(global.pdfExporter instanceof global.PdfExporter)) {
                    global.pdfExporter = new global.PdfExporter();
                }
                global.pdfExporter.exportToPdf();
            } catch (error) {
                console.error('[AppActions] PDF 导出失败:', error);
                if (typeof global.showMessage === 'function') {
                    global.showMessage('PDF 导出失败,请稍后重试', 'error');
                }
            }
        }).catch(function handleExportError(error) {
            console.error('[AppActions] 导出失败:', error);
    ...
    global.AppActions = Object.assign({}, global.AppActions, {
        ...
        exportPracticePdf: exportPracticePdf,
