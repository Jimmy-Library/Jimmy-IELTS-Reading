...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('.lookup-section--english h3').filter({ hasText: '英英释义与例句' }).count(), 1);
    assert.ok(await page.locator('.lookup-section--chinese .lookup-sense-example').count() >= 2);
    assert.ok(await page.locator('.lookup-section--chinese [data-pos-group="noun"] .lookup-sense-example').count() >= 1);
    assert.ok(await page.locator('.lookup-section--chinese [data-pos-group="verb"] .lookup-sense-example').count() >= 1);
    assert.ok(await page.locator('.lookup-section--english [data-pos-group="noun"]').count() >= 1);
    assert.ok(await page.locator('.lookup-section--english [data-pos-group="verb"]').count() >= 1);
    assert.equal((await page.locator('.lookup-section--chinese').innerText()).includes('ORIGINAL RICH PASSAGE SENTENCE'), false);
