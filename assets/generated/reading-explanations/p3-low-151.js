(function registerReadingExplanationData(global) {
  'use strict';
  if (!global.__READING_EXPLANATION_DATA__ || typeof global.__READING_EXPLANATION_DATA__.register !== "function") {
    throw new Error("reading_explanation_registry_missing");
  }
  global.__READING_EXPLANATION_DATA__.register("p3-low-151", {
  "schemaVersion": "ReadingExplanationV1",
  "examId": "p3-low-151",
  "meta": {
    "examId": "p3-low-151",
    "title": "Book Review The Discovery of Slowness 富兰克林(慢的发现)",
    "category": "P3",
    "sourceDoc": "P3 解析+1202高频_副本.md",
    "noteType": "总结",
    "matchedTitle": "Book Review The Discovery of Slowness 富兰克林 (慢的发现)"
  },
  "passageNotes": [
    {
      "label": "Paragraph A",
      "text": "英国探险家、作家约翰・富兰克林（John Franklin，1786-1847）14 岁参军，参加特拉法加海战。和平时期转向北极探险，致力于寻找 “西北航道”（连接美洲东西海岸的传说航道）。1819-1822 年，他带领 20 人团队完成艰苦的陆路探险，徒步 8880 公里，测绘数百公里未知海岸线，但返程时断粮，被迫煮皮带靴子充饥。"
    },
    {
      "label": "Paragraph B",
      "text": "此后富兰克林以 “吃靴子的人” 为标签，成为旅行作家和演说家。1845 年，他率 “厄瑞玻斯号”“恐怖号” 两艘船及 129 名船员再次前往北极，此后 14 年杳无音讯，30 多支搜救队无果。最终发现，船队因船只被困冰中，全员遇难。"
    },
    {
      "label": "Paragraph C",
      "text": "从富兰克林的书信和回忆录可见，他专注于战争与探险的外部职责，极少内省，这种 “平淡特质” 使其成为小说家易塑造的角色。 Sten Nadolny 在小说《慢的发现》中充分利用这一点，为富兰克林赋予了一个无历史依据的核心特质 ——“Langsamkeit”（缓慢 / 沉静）。"
    },
    {
      "label": "Paragraph D",
      "text": "“缓慢” 影响富兰克林的行为、视野、思维与语言。小说开篇描写少年富兰克林因反应慢接不到球，虽遭同伴嘲笑，仍拒绝 “跟风行事”。Nadolny 认为，富兰克林对北极的痴迷源于 “寻找适配自身缓慢特质的环境”—— 他梦想北方 “没有时辰与昼夜的时空”，一个 “无人觉得他慢” 的地方。"
    },
    {
      "label": "Paragraph E",
      "text": "冰是缓慢的象征 —— 阿尔卑斯冰裂缝中的蓝冰源于数十年前的降雪，极地浮冰形成至少需两年（海水先结薄冰，再固化为厚冰）。冰要求探险者具备相应耐心，高纬度地区成功的探险家多非行动迅速者，而是拥有非凡镇定、耐枯燥、能承受苦难的品质。"
    },
    {
      "label": "Paragraph F",
      "text": "历史上的富兰克林本就具备这些品质，Nadolny 的夸张塑造并不突兀。成年富兰克林因思维慢无法流利表达，便背诵成套词句回应，说话缓慢零散；在海军中，他 “先思考后行动” 的风格起初遭嘲笑，但坚持自我后逐渐赢得尊重 —— 面对要求加快战报的 commodore（海军准将），他回应 “我讲述时，有自己的节奏”；一位中尉称赞 “富兰克林虽慢，却从不浪费时间”。"
    },
    {
      "label": "Paragraph G",
      "text": "Nadolny 还将 “缓慢” 的核心隐喻融入小说语言：描写富兰克林早年的章节由零散片段、反问句、联想跳跃和感叹号构成；叙述首次北极探险的章节，叙事节奏贴合冰封景观的韵律及 “饥饿催生的缓慢”。"
    },
    {
      "label": "Paragraph H",
      "text": "《慢的发现》1983 年在德国出版后，销量超百万册，被译为 15 种语言，被欧洲多个压力团体和机构（涉及可持续发展、管理学、交通政策等）视为手册宣言，甚至参与德国道路限速辩论。这些团体的共同点是 “反对后现代高速文化”。Nadolny 笔下的富兰克林吸引他们，因其不受 “必须忙碌”“快则优” 的观念束缚。对管理者而言，小说中关于时间效率、严谨性和盈利能力的论述（如 “‘太晚’意味着‘等待时间不够长’”）极具吸引力。"
    }
  ],
  "questionExplanations": [
    {
      "sectionTitle": "1. 段落匹配（Questions 27–31：Which paragraph contains the following information?）",
      "mode": "group",
      "items": [
        {
          "questionNumber": 27,
          "text": "（1）题目 27：a summary of Franklin’s occupation in the period before his first expedition（富兰克林首次探险前的职业概述）\n答案：A\n解析：定位 Paragraph A 中 “joined the Navy at the age of 14, and fought at the battle of Trafalgar. When peace came, he turned his attention to Arctic exploration”，明确提到首次探险（1819 年）前 “参军、参加海战、转向北极探索” 的职业经历，与题干对应，因此答案为 A。",
          "questionId": "q1"
        },
        {
          "questionNumber": 28,
          "text": "（2）题目 28：a reference to a feature of Franklin’s character in Nadolny’s novel that has no definite basis in fact（提及 Nadolny 小说中富兰克林的某一性格特征无确凿史实依据）\n答案：C\n解析：定位 Paragraph C 中 “Nadolny has endowed John Franklin with a defining trait for which there is no historical evidence: Langsamkeit (‘slowness’, or ‘calmness’)”，明确提到 “缓慢” 这一性格特征 “无历史依据”，与题干对应，因此答案为 C。",
          "questionId": "q2"
        },
        {
          "questionNumber": 29,
          "text": "（3）题目 29：a connection between the central theme of the novel and an environmental process（小说核心主题与某一自然过程的关联）\n答案：E\n解析：定位 Paragraph E 中 “Ice is a slow mover... Polar pack ice takes at least two years to form... Ice demands a corresponding patience from those who venture onto it”，将 “冰层缓慢形成” 这一自然过程与小说 “缓慢” 主题关联，与题干对应，因此答案为 E。",
          "questionId": "q3"
        },
        {
          "questionNumber": 30,
          "text": "（4）题目 30：a reference to the widespread appeal of Nadolny’s novel（提及 Nadolny 小说的广泛吸引力）\n答案：H\n解析：定位 Paragraph H 中 “has sold more than a million copies and been translated into 15 languages. It has been adopted as a manual and manifesto by European pressure groups and institutions representing causes as diverse as sustainable development, management science and motoring policy”，明确提到 “销量、译本、多领域应用”，体现广泛吸引力，与题干对应，因此答案为 H。",
          "questionId": "q4"
        },
        {
          "questionNumber": 31,
          "text": "（5）题目 31：a summary of events following Franklin’s return from his first expedition（富兰克林首次探险归来后的事件概述）\n答案：B\n解析：定位 Paragraph B 中 “There followed a career as a travel writer and public speaker... Then in 1845, Franklin set off back to the Arctic... Nothing was heard of them for 14 years... eventually it was discovered that Franklin and all his team had perished”，明确概述 “成为作家演说家、1845 年再次探险、失踪遇难”，与题干对应，因此答案为 B。",
          "questionId": "q5"
        }
      ],
      "questionRange": {
        "start": 27,
        "end": 31
      },
      "text": "答案：A\n解析：定位 Paragraph A 中 “joined the Navy at the age of 14, and fought at the battle of Trafalgar. When peace came, he turned his attention to Arctic exploration”，明确提到首次探险（1819 年）前 “参军、参加海战、转向北极探索” 的职业经历，与题干对应，因此答案为 A。\n答案：C\n解析：定位 Paragraph C 中 “Nadolny has endowed John Franklin with a defining trait for which there is no historical evidence: Langsamkeit (‘slowness’, or ‘calmness’)”，明确提到 “缓慢” 这一性格特征 “无历史依据”，与题干对应，因此答案为 C。\n答案：E\n解析：定位 Paragraph E 中 “Ice is a slow mover... Polar pack ice takes at least two years to form... Ice demands a corresponding patience from those who venture onto it”，将 “冰层缓慢形成” 这一自然过程与小说 “缓慢” 主题关联，与题干对应，因此答案为 E。\n答案：H\n解析：定位 Paragraph H 中 “has sold more than a million copies and been translated into 15 languages. It has been adopted as a manual and manifesto by European pressure groups and institutions representing causes as diverse as sustainable development, management science and motoring policy”，明确提到 “销量、译本、多领域应用”，体现广泛吸引力，与题干对应，因此答案为 H。\n答案：B\n解析：定位 Paragraph B 中 “There followed a career as a travel writer and public speaker... Then in 1845, Franklin set off back to the Arctic... Nothing was heard of them for 14 years... eventually it was discovered that Franklin and all his team had perished”，明确概述 “成为作家演说家、1845 年再次探险、失踪遇难”，与题干对应，因此答案为 B。"
    },
    {
      "sectionTitle": "2. 摘要选词填空（Questions 32–35：Choose A–L）",
      "mode": "group",
      "items": [
        {
          "questionNumber": 32,
          "text": "（1）题目 32：Little is known from Franklin’s correspondence and published writings about his \\___\\___.\n题目翻译：从富兰克林的书信和出版作品中，人们对他的_\\___\\__知之甚少。\n答案：G（personality，个性）\n解析：定位 Paragraph C 中 “Franklin comes across as a man dedicated to the external duties... who kept introspection and self-analysis to a minimum. His blandness”，明确提到书信和回忆录 “极少提及内省，无法体现个性”，因此选 G。",
          "questionId": "q6"
        },
        {
          "questionNumber": 33,
          "text": "（2）题目 33：The author, Nadolny, suggests that Franklin’s whole life can be explained by the quality of ‘slowness’. He begins the book by showing how this affects the young Franklin’s \\___\\___ activities.\n题目翻译：作者 Nadolny 认为，“缓慢” 这一特质可解释富兰克林的一生。他在书中开篇描写这一特质如何影响少年富兰克林的_\\___\\__活动。\n答案：K（sporting，体育）\n解析：定位 Paragraph D 中 “The opening scene... depicts Franklin as a young boy, failing to catch a ball because his reaction time is too slow”，“接球” 属于体育活动，因此选 K。",
          "questionId": "q7"
        },
        {
          "questionNumber": 34,
          "text": "（3）题目 34：and makes him become \\___\\___.\n题目翻译：并使他成为_\\___\\__。\n答案：F（an outsider，局外人）\n解析：定位 Paragraph D 中 “despite the bullying of his peers, Franklin resolves not to fall into step with ‘their way of doing things’”，“拒绝跟风” 意味着成为 “局外人”，因此选 F。",
          "questionId": "q8"
        },
        {
          "questionNumber": 35,
          "text": "（4）题目 35：As a child, it is the quality of \\___\\___ that attracts Franklin to the Arctic.\n题目翻译：童年时，是_\\___\\__这一特质吸引富兰克林向往北极。\n答案：L（timelessness，无时间性）\n解析：定位 Paragraph D 中 “Franklin as a boy dreaming of the ‘time without hours and days’ which exists in the far north”，“没有时辰与昼夜” 对应 “timelessness（无时间性）”，因此选 L。",
          "questionId": "q9"
        }
      ],
      "questionRange": {
        "start": 32,
        "end": 35
      },
      "text": "题目翻译：从富兰克林的书信和出版作品中，人们对他的_\\___\\__知之甚少。\n答案：G（personality，个性）\n解析：定位 Paragraph C 中 “Franklin comes across as a man dedicated to the external duties... who kept introspection and self-analysis to a minimum. His blandness”，明确提到书信和回忆录 “极少提及内省，无法体现个性”，因此选 G。\n题目翻译：作者 Nadolny 认为，“缓慢” 这一特质可解释富兰克林的一生。他在书中开篇描写这一特质如何影响少年富兰克林的_\\___\\__活动。\n答案：K（sporting，体育）\n解析：定位 Paragraph D 中 “The opening scene... depicts Franklin as a young boy, failing to catch a ball because his reaction time is too slow”，“接球” 属于体育活动，因此选 K。\n题目翻译：并使他成为_\\___\\__。\n答案：F（an outsider，局外人）\n解析：定位 Paragraph D 中 “despite the bullying of his peers, Franklin resolves not to fall into step with ‘their way of doing things’”，“拒绝跟风” 意味着成为 “局外人”，因此选 F。\n题目翻译：童年时，是_\\___\\__这一特质吸引富兰克林向往北极。\n答案：L（timelessness，无时间性）\n解析：定位 Paragraph D 中 “Franklin as a boy dreaming of the ‘time without hours and days’ which exists in the far north”，“没有时辰与昼夜” 对应 “timelessness（无时间性）”，因此选 L。"
    },
    {
      "sectionTitle": "3. 单选题（Questions 36–40：Choose A–D）",
      "mode": "per_question",
      "items": [
        {
          "questionNumber": 36,
          "text": "（1）题目 36：In paragraph E, the writer describes different types of ice in order to illustrate（E 段中，作者描述不同类型的冰是为了说明什么？）\n答案：D（the personal qualities required of an explorer in Arctic conditions. 北极环境下探险者所需的个人品质）\n解析：定位 Paragraph E 中 “Ice demands a corresponding patience from those who venture onto it. The explorers who have thrived at high latitudes... have tended instead to demonstrate unusual self-possession, a considerable capacity for boredom, and a talent for the uncomplaining endurance of suffering”，描述冰的 “缓慢” 是为了引出 “探险者需具备耐心、镇定等品质”，因此选 D。",
          "questionId": "q10"
        },
        {
          "questionNumber": 37,
          "text": "（2）题目 37：What is said about the way Franklin communicates?（关于富兰克林的沟通方式，文中提到什么？）\n答案：C（He uses words and phrases that he has previously memorised. 他使用之前背诵过的词句）\n解析：定位 Paragraph F 中 “Franklin's slowness of thought means that he is unable to speak fluently, so he learns by heart 'entire fleets of words and batteries of responses', and speaks a languid, bric-à-brac language”，明确提到 “因思维慢无法流利表达，便背诵词句回应”，因此选 C。",
          "questionId": "q11"
        },
        {
          "questionNumber": 38,
          "text": "（3）题目 38：How does the attitude of the other sailors change towards Franklin?（其他水手对富兰克林的态度如何变化？）\n答案：D（They eventually appreciate his good qualities. 他们最终欣赏他的优点）\n解析：定位 Paragraph F 中 “his method of thinking first and acting later initially provokes mockery... Franklin persists in doing things his way, and gradually earns the respect of those around him... A lieutenant says approvingly of him: 'Because Franklin is so slow, he never loses time'”，明确提到 “从嘲笑到尊重欣赏”，因此选 D。",
          "questionId": "q12"
        },
        {
          "questionNumber": 39,
          "text": "（4）题目 39：The chapters of The Discovery of Slowness which describe Franklin's early years contain（《慢的发现》中描写富兰克林早年的章节包含什么？）\n答案：B（short, disconnected pieces of text. 简短、不连贯的文本片段）\n解析：定位 Paragraph G 中 “The chapters describing Franklin's early years are a medley of separate fragments, rhetorical questions, associative jumps and exclamation marks”，“零散片段、联想跳跃” 对应 “简短不连贯的文本”，因此选 B。",
          "questionId": "q13"
        },
        {
          "questionNumber": 40,
          "text": "（5）题目 40：The Discovery of Slowness has achieved widespread popularity because（《慢的发现》广受欢迎的原因是什么？）\n答案：A（it reflects many people's concern about one aspect of modern life. 它反映了许多人对现代生活某一方面的担忧）\n解析：定位 Paragraph H 中 “The various groups that have taken the novel up have one thing in common: a dislike of the high-speed culture of Postmodernity. Nadolny's Franklin appeals to them because he is immune to 'the compulsion to be constantly occupied', and to the idea that 'someone was better if he could do the same thing fast'”，明确提到 “小说反对高速文化，反映了人们对现代高速生活的担忧”，因此选 A。",
          "questionId": "q14"
        }
      ],
      "questionRange": {
        "start": 36,
        "end": 40
      },
      "text": "答案：D（the personal qualities required of an explorer in Arctic conditions. 北极环境下探险者所需的个人品质）\n解析：定位 Paragraph E 中 “Ice demands a corresponding patience from those who venture onto it. The explorers who have thrived at high latitudes... have tended instead to demonstrate unusual self-possession, a considerable capacity for boredom, and a talent for the uncomplaining endurance of suffering”，描述冰的 “缓慢” 是为了引出 “探险者需具备耐心、镇定等品质”，因此选 D。\n答案：C（He uses words and phrases that he has previously memorised. 他使用之前背诵过的词句）\n解析：定位 Paragraph F 中 “Franklin’s slowness of thought means that he is unable to speak fluently, so he learns by heart ‘entire fleets of words and batteries of responses’, and speaks a languid, bric-à-brac language”，明确提到 “因思维慢无法流利表达，便背诵词句回应”，因此选 C。\n答案：D（They eventually appreciate his good qualities. 他们最终欣赏他的优点）\n解析：定位 Paragraph F 中 “his method of thinking first and acting later initially provokes mockery... Franklin persists in doing things his way, and gradually earns the respect of those around him... A lieutenant says approvingly of him: ‘Because Franklin is so slow, he never loses time’”，明确提到 “从嘲笑到尊重欣赏”，因此选 D。\n答案：B（short, disconnected pieces of text. 简短、不连贯的文本片段）\n解析：定位 Paragraph G 中 “The chapters describing Franklin’s early years are a medley of separate fragments, rhetorical questions, associative jumps and exclamation marks”，“零散片段、联想跳跃” 对应 “简短不连贯的文本”，因此选 B。\n答案：A（it reflects many people’s concern about one aspect of modern life. 它反映了许多人对现代生活某一方面的担忧）\n解析：定位 Paragraph H 中 “The various groups that have taken the novel up have one thing in common: a dislike of the high-speed culture of Postmodernity. Nadolny’s Franklin appeals to them because he is immune to ‘the compulsion to be constantly occupied’, and to the idea that ‘someone was better if he could do the same thing fast’”，明确提到 “小说反对高速文化，反映了人们对现代高速生活的担忧”，因此选 A。"
    }
  ]
}
  );
})(typeof window !== "undefined" ? window : globalThis);
